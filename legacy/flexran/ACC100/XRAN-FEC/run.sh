...
annotations_file="`/bin/pwd`/xran-annotations.json"  # for XRAN, use xran-annotations.json
...
if [ ! -z "$annotations_file" ]; then
    if [ $(basename -- $annotations_file) == "xran-annotations.json" ]; then
        ...
        # Blindly delete the namespace just in case the previous run aborted,
        ...
        if [ "$kind"  != "null" ]; then
            ...
            echo "Please re-provision SRIOV which may have been deleted after the previous run"
            exit
        fi
