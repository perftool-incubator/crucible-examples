...
#volumeMounts_file="`/bin/pwd`/volumeMounts.json"       <=== For v20.3 
...
endpoint_opt+=",userenv:$userenv,cpu-partitioning:default:1,resources:client-1:$resources_file,annotations:client-1:$annotations_file,securityContext:client-1:$securityContext_file,hugepage:1"

if [ ! -z $volumeMounts_file ]; then
   endpoint_opt+=",volumeMounts:client-1:$volumeMounts_file,volumes:client-1:$volumes_file"
fi
